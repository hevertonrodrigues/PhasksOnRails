class TasksListsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @tasks_list.save
        ...
        format.json { render action: 'show', status: :created, location: @tasks_list }
      else
  ...
  def destroy
    @tasks_list.destroy
    respond_to do |format|
      format.html { redirect_to tasks_lists_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def tasks_list_params
      params.require(:tasks_list).permit(:name, :description)
    end
end
